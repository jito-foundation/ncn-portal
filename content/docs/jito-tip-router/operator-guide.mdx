...
### SystemD Service Configuration Update

1. Update OPERATOR_ADDRESS to the account created during Registration
2. Update KEYPAIR_PATH to reference the keypair designated as `voter` on the Operator account
    - Verify keypair public key with
    ```bash
    jito-restaking-cli restaking operator get <OPERATOR_ADDRESS>
    ```
3. Update NCN_ADDRESS=`jtoF4epChkmd75V2kxXSmywatczAomDqKu6VfWUQocT`
4. Update TIP_ROUTER_PROGRAM_ID=`RouterBmuRBkPUbgEDMtdvTZ75GBdSREZR5uGUxxxpb`

### Service Preparation

The service must be operational for a minimum of 10 minutes before epoch 735 commencement for setup and incremental snapshot backup.

#### If excessive disk usage has been observed in the /accounts directory and node stoppage is feasible: